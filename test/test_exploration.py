...
class TestBlockchain(unittest.TestCase):
    sample_accounts_by_network = {
        ...
        Network.MATIC: "0x3AFac8309C2a93406c326dbb9Ab2578F3ed165d9",
    ...
    sample_contracts_by_network = {
        ...
        Network.MATIC: "0x3011810abfec25777a01d5fbef08b2ad12860460",
    ...
    sample_txn_by_network = {
        ...
        Network.MATIC: "0xd3d4de1612017d7e8c69c70879ab4ba3b2f4e2c638f05b5d13b2c417e46f5be2",
    ...
    def assert_url_validity(self, explorer_url: Optional[str], expected_text: str):
        ...
        self.assertNotIn(
            "//",
            parsed.path,
            msg=f"There should not be a double-backslash: {explorer_url}",
        )
        self.assertTrue(parsed.scheme, "A complete URL should have an access scheme")
        self.assertTrue(
            parsed.netloc, "A complete URL should have a domain name or IP address"
        )
        self.assertTrue(
            parsed.path,
            "It's improbable that there's a domain wholly devoted to one token "
        ...
        try:
            response = requests.get(
                url=explorer_url,
                timeout=10,
                headers={"User-Agent": "Sweet.io test", "Accept": "application/json"},
            )
        except ReadTimeout:
            logger.warning(
                "A connection error prevented us from receiving a response from %s",
                explorer_url,
            )
        ...
        self.assertTrue(
            response.text,
            msg="Why was the response empty upon contacting {}".format(explorer_url),
        )
        loading_js = "without JavaScript enabled" in response.text
        if loading_js:
            logger.info(
                "The page is still loading, so we're unable to assert whether it's actually relevant to us. "
    ...
    def test_get_explorer_url_for_account(self):
        ...
        for network, address in self.sample_accounts_by_network.items():
            ...
            self.assert_url_validity(
                explorer_url=url, expected_text="transaction"
            )  # Transactions are mentioned for account overview
    ...
    def test_get_explorer_url_for_token_wallet(self):
        ...
        for network, address in self.sample_accounts_by_network.items():
            url = get_explorer_url_for_account(network=network, address=address)
            self.assert_url_validity(
                explorer_url=url, expected_text="hash"
    ...
    def test_get_explorer_url_for_nft_contract(self):
        ...
        for network, contract in self.sample_contracts_by_network.items():
            url = get_explorer_url_for_nft_contract(
                network=network, contract_address=contract
            )
            self.assert_url_validity(
                explorer_url=url, expected_text="hash"
            )  # Hashes for transactions
    ...
    def test_get_explorer_url_for_token(self):
        results = set()
        for network, contract in self.sample_contracts_by_network.items():
            url = get_explorer_url_for_token(
                network=network, address=contract, token_id="1"
            )
            self.assert_url_validity(
                explorer_url=url, expected_text="hash"
            )  # Hashes for transactions
